 ...
 * typically need to handle the events in the UI thread, so it will typically
 * post the events in an even loop, to handle them from the UI thread. In that
 * case, be careful to always post the event in the event loop, even if the
 ...
 * Go to a give index.
 *
 * Return VLC_EGENERIC if the index is not either -1 or in range (less than
 * vlc_playlist_Count()).
 *
 * \param playlist the playlist, locked
