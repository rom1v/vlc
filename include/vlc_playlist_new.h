...
struct vlc_playlist_callbacks
{
     ...
     * \param items    the array of moved items
 ...
 * the expected index of the first item of the slice (as known from the
 * client). Hopefully, the index should often match, since conflicts are
 * expected to be rare. Pass -1 not to pass any hint.
 *
 * \param playlist   the playlist, locked
