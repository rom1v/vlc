...
# define vlc_player_Lock(p)
# define vlc_player_Unlock(p)
                                 ...
                                 struct vlc_playlist_state *saved_state)
{
    if (saved_state->current != playlist->current)
        PlaylistNotify(playlist, on_current_item_changed, playlist->current);
...
test_append(void)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        int ret = vlc_playlist_Append(playlist, media[i]);
        assert(ret == VLC_SUCCESS);
    }

    /* append several at once */
    int ret = vlc_playlist_AppendAll(playlist, &media[5], 5);
    assert(ret == VLC_SUCCESS);
...
test_insert(void)
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        int ret = vlc_playlist_Insert(playlist, 2, media[i + 5]);
        assert(ret == VLC_SUCCESS);
    }

    /* insert several at once */
    int ret = vlc_playlist_InsertAll(playlist, 6, &media[10], 5);
    assert(ret == VLC_SUCCESS);
...
test_remove(void)
{
    ...
    /* remove one by one */
    for (int i = 0; i < 3; ++i)
        vlc_playlist_Remove(playlist, 2);

    /* remove several at once */
    vlc_playlist_RemoveSlice(playlist, 3, 2);
...
static void
callback_on_current_item_changed(vlc_playlist_t *playlist, ssize_t index,
                                 void *userdata)
{
    PlaylistAssertLocked(playlist);
    struct callback_ctx *ctx = userdata;
...
test_items_added_callbacks(void)
{
    ...
    struct vlc_playlist_callbacks cbs = {
        ...
        .on_current_item_changed = callback_on_current_item_changed,
    ...
    int ret = vlc_playlist_Append(playlist, media[0]);
    assert(ret == VLC_SUCCESS);
    ...
    ret = vlc_playlist_InsertAll(playlist, 0, &media[1], 4);
    assert(ret == VLC_SUCCESS);
    ...
    ret = vlc_playlist_AppendAll(playlist, &media[5], 5);
    assert(ret == VLC_SUCCESS);
...
test_items_removed_callbacks(void)
{
    ...
    vlc_playlist_Remove(playlist, 4);
    ...
    vlc_playlist_RemoveSlice(playlist, 2, 4);
    ...
    vlc_playlist_RemoveSlice(playlist, 0, 5);
...
test_prev(void)
{
    ...
    int ret = vlc_playlist_Prev(playlist);
    assert(ret == VLC_SUCCESS);
...
test_next(void)
{
    ...
    int ret = vlc_playlist_Next(playlist);
    assert(ret == VLC_SUCCESS);
...
test_goto(void)
{
    ...
    int ret = vlc_playlist_GoTo(playlist, 4);
    assert(ret == VLC_SUCCESS);
