...
PlaylistHasNext(vlc_playlist_t *playlist)
{
    ...
    {
        case VLC_PLAYLIST_PLAYBACK_NORMAL:
        case VLC_PLAYLIST_PLAYBACK_LOOP_ON_CURRENT:
            /* not the last item */
            return playlist->current < (ssize_t) playlist->items.size - 1;
        case VLC_PLAYLIST_PLAYBACK_LOOP_ON_ALL:
            /* not empty */
...
static input_item_t *
player_get_next_item(vlc_player_t *player, void *userdata)
{
    ...
    if (PlaylistHasNext(playlist))
    {
        ...
        item = playlist->items.data[index]->input;
    }
    else
        item = NULL;
...
static const struct vlc_player_owner_cbs player_owner_callbacks = {
    .get_next_item = player_get_next_item,
};
...
vlc_playlist_Clear(vlc_playlist_t *playlist)
{
    ...
    bool had_prev = playlist->has_prev;
    bool had_next = playlist->has_next;
    ...
    if (had_prev)
        NotifyHasPrevChanged(playlist, false);
    if (had_next)
        NotifyHasNextChanged(playlist, false);
}
...
vlc_playlist_Next(vlc_playlist_t *playlist)
{
    ...
    NotifyCurrentItemChanged(playlist, index, playlist->items.data[index]);
    if (playlist->has_next != had_next)
        NotifyHasNextChanged(playlist, playlist->has_next);
...
vlc_playlist_Prev(vlc_playlist_t *playlist)
{
    ...
    NotifyCurrentItemChanged(playlist, index, playlist->items.data[index]);
    if (playlist->has_prev != had_prev)
        NotifyHasPrevChanged(playlist, playlist->has_prev);
...
test_expand_item(void)
{
    ...
    input_item_t *item_to_expand = playlist->items.data[8]->input;
