                                            ...
                                            void *userdata)
{
    ...
    if (!new_media)
        /* the player has stopped, keep the playlist state unchanged */
        return;
...
player_get_next_media(vlc_player_t *player, void *userdata)
{
    ...
    if (PlaylistHasNext(playlist))
    {
        ...
        return playlist->items.data[index]->media;
    }
                         ...
                         void *userdata)
{
    ...
    /* notify the listener with the initial state */
    if (listener->cbs->on_items_reset)
        listener->cbs->on_items_reset(playlist, playlist->items.data,
                                      playlist->items.size, listener->userdata);
    return listener;
}
    ...
    for (size_t i = 0; i < req->count; ++i)
    {
        ...
        switch (criterion->key)
        {
            case VLC_PLAYLIST_SORT_KEY_TITLE:
                ret = MediaCompareTitlesOrNames(a->media, b->media);
                break;
            case VLC_PLAYLIST_SORT_KEY_DURATION:
                ret = MediaCompareDurations(a->media, b->media);
                break;
            default:
        ...
        if (ret)
        {
            if (criterion->order == VLC_PLAYLIST_SORT_ORDER_DESCENDING)
                /* undefined if ret == INT_MIN, but it's never the case */
                return -ret;
            return ret;
        }
    }
                              ...
                              enum vlc_playlist_playback_order order)
{
    ...
    if (order == VLC_PLAYLIST_PLAYBACK_ORDER_RANDOM)
    {
        /* randomizer expected to be empty at this point */
...
test_request_remove_adapt(void)
{
    ...
    vlc_playlist_item_t *items_to_remove[] = {
        ...
        vlc_playlist_Get(playlist, 10), /* inexistant */
...
test_request_move_adapt(void)
{
    ...
    vlc_playlist_item_t *items_to_move[] = {
        ...
        vlc_playlist_Get(playlist, 15), /* inexistant */
        vlc_playlist_Get(playlist, 3),
