...
vlc_playlist_Export(struct vlc_playlist_view *playlist_view,
                    ...
                    const char *type)
{
    vlc_playlist_AssertLocked(playlist_view->playlist);

    // XXX what vlc_object_t should we use?
    vlc_object_t *obj = (vlc_object_t *) playlist_view->playlist->player;

    struct playlist_export *export =
        vlc_custom_create(obj, sizeof(*export), "playlist export");
