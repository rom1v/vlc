                    ...
                    const struct vlc_input_event *event, void *user_data)
{
    ...
    switch (event->type)
    {
        ...
        case INPUT_EVENT_SUBS_FPS:
            input->subs_fps = event->subs_fps;
            vlc_player_SendEvent(player, on_associated_subs_fps_changed,
                                 input->subs_fps);
        default:
            break;
    }
...
vlc_player_vout_SetWallpaperModeEnabled(vlc_player_t *player, bool enabled)
{
    ...
    vlc_player_SendEvent(player, on_vout_wallpaper_mode_changed, enabled);
}
