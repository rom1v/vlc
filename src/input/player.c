...
vlc_player_title_list_Create(const input_title_t **array, size_t count)
{
    ...
    for (size_t title_idx = 0; title_idx < titles->count; ++title_idx)
    {
        ...
        title->name = strdup(input_title->psz_name);
        ...
        if (chapters)
        {
                 ...
                 ++chapter_idx)
            {
                ...
                chapter->name = strdup(seekpoint->psz_name);
        ...
        if (!title->name || (!title->chapters && input_title->i_seekpoint > 0))
        {
                                    ...
                                    const struct vlc_input_event_program *ev)
{
    ...
    switch (ev->action)
    {
        ...
        {
            const struct vlc_player_program *unselected = NULL;
            const struct vlc_player_program *selected = NULL;
