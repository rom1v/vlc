...
vlc_player_input_Delete(struct vlc_player_input *input)
{
    ...
    for (size_t prgm_idx = 0; prgm_idx < input->program_vector.size; ++prgm_idx)
        vlc_player_program_Destroy(input->program_vector.data[prgm_idx]);
                                  ...
                                  input_state_e state)
{
     ...
     * playing. */
    if (player->next_media && input->started
                     ...
                     enum vlc_player_seek_whence whence)
{
    ...
    if (input && input->capabilities & VLC_INPUT_CAPABILITIES_SEEKABLE)
