...
media_tree_t *media_tree_Create( const media_tree_callbacks_t *p_callbacks, void *userdata )
{
...
#ifndef NDEBUG
    /* Check that the parent belonged to the tree */
    for( media_node_t *p = p_parent; p != &p_tree->p_root; p = p->p_parent )
        assert( p );
#endif

    if( p_priv->callbacks.pf_node_added )
        p_priv->callbacks.pf_node_added( p_tree, p_parent, p_node );
...
media_node_t *media_tree_Remove( media_tree_t *p_tree, media_node_t *p_input )
{
    AssertLocked( p_tree );
    // TODO
    return NULL;
