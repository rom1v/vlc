...
To solve the issue, we decided to try to apply the request as best effort: find
the items if they had been moved, ignore the items not found for removal, etc.
...
to a "direct" range removal (concretely, we would like to avoid a linear lookup
for every item).
...
In the functions to insert, move (not yet) and remove, the items are selected by
index, which must be valid (in range). If a client wants to apply more complex
changes based on items (see [synchronization](#synchronization)), they can lock
the playlist, find the items they want, apply their changes, then unlock the
mutex.
...
The items are stored in a contiguous dynamic array ([`vlc_vector`]).
...
It also exposes methods to request best effort appending, insertions and
removals (see [UI to core](#ui-to-core)).

_These best-effort algorithms should probably be moved to the core, so that
several UI may benefit from them. However, I'm reluctant to include them in the
core playlist API, since they are just independant helpers. Maybe they could be
exposed separately._
...
The playlist may be modified by the "best effort" methods (see [UI to
core](#ui-to-core)) exposed (currently) in the `Playlist` class. The _list
model_ will be modified indirectly via the core playlist callbacks.
...
will be executed during the call. In that case, the event must be posted to the
