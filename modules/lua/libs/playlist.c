                               ...
                               enum vlc_playlist_playback_repeat enabled_mode)
{
    ...
    bool toggle = top == 0;
    bool enable = (toggle && repeat != enabled_mode) || take_bool(L);
...
static int vlclua_playlist_random(lua_State *L)
{
    ...
    bool toggle = top == 0;
    bool enable = (toggle && order != VLC_PLAYLIST_PLAYBACK_ORDER_RANDOM)
                  || take_bool(L);
...
static int vlclua_playlist_add_common(lua_State *L, bool play)
{
    ...
    while (lua_next(L, -2))
    {
        ...
        if (item != NULL)
        {
            bool ok = vlc_playlist_AppendOne(playlist, item);
            if (ok)
            {
                count++;
                if (play)
                {
                    ...
                    vlc_playlist_GoTo(playlist, last);
                    vlc_playlist_Start(playlist);
                }
            }
