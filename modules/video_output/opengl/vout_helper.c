                    ...
                    bool b_dump_shaders)
{
    ...
    struct vlc_gl_interop *interop =
        CreateInterop(vgl->gl, &vgl->vt, context, glexts, fmt, subpics);
    if (!interop)
    {
        free(tc);
        return VLC_ENOMEM;
...
#undef GET_PROC_ADDR
    ...
    {
        ret = vout_display_opengl_GenTextures(vgl->prgm->tc->interop,
                                      ...
                                      subpicture_t *subpicture)
{
    ...
    if (subpicture) {
        ...
        for (subpicture_region_t *r = subpicture->p_region;
            ...
            if (!glr->texture)
            {
                ...
                int ret = vout_display_opengl_GenTextures(interop, &glr->width,
                                                          &glr->height,
                                                          &glr->texture);
                if (ret != VLC_SUCCESS)
                    break;
    ...
    for (int i = 0; i < last_count; i++) {
        if (last[i].texture)
            vout_display_opengl_DelTextures(interop, &last[i].texture);
    }
    free(last);
