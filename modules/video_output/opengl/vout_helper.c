...
GenTextures(const opengl_tex_converter_t *tc,
            ...
            GLuint *textures)
{
    const struct vlc_gl_importer *importer = &tc->importer;

    tc->vt->GenTextures(importer->tex_count, textures);

    for (unsigned i = 0; i < importer->tex_count; i++)
    {
        tc->vt->BindTexture(importer->tex_target, textures[i]);

#if !defined(USE_OPENGL_ES2)
        /* Set the texture parameters */
        tc->vt->TexParameterf(importer->tex_target, GL_TEXTURE_PRIORITY, 1.0);
        tc->vt->TexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
#endif

        tc->vt->TexParameteri(importer->tex_target, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        tc->vt->TexParameteri(importer->tex_target, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        tc->vt->TexParameteri(importer->tex_target, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        tc->vt->TexParameteri(importer->tex_target, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    }

    if (importer->ops->alloc_textures != NULL)
    {
        int ret = importer->ops->alloc_textures(importer, textures, tex_width,
                                                tex_height);
        if (ret != VLC_SUCCESS)
        {
            tc->vt->DeleteTextures(importer->tex_count, textures);
...
#   endif
    ...
    assert(tc->fshader != 0 && tc->importer.tex_target != 0 &&
           tc->importer.tex_count > 0 && tc->importer.ops->import != NULL &&
           tc->pf_fetch_locations != NULL && tc->pf_prepare_shader != NULL);
...
#undef GET_PROC_ADDR
    ...
    for (unsigned j = 0; j < tc->importer.tex_count; j++) {
        const GLsizei w = vgl->fmt.i_visible_width  * tc->importer.cfg[j].w.num
                        / tc->importer.cfg[j].w.den;
        const GLsizei h = vgl->fmt.i_visible_height * tc->importer.cfg[j].h.num
                        / tc->importer.cfg[j].h.den;
                                ...
                                picture_t *picture, subpicture_t *subpicture)
{
    ...
    opengl_tex_converter_t *tc = vgl->prgm->tc;
    struct vlc_gl_importer *importer = &tc->importer;

    /* Update the texture */
    int ret = importer->ops->import(importer, vgl->texture, vgl->tex_width,
    ...
    if (subpicture) {
        ...
        for (subpicture_region_t *r = subpicture->p_region;
            ...
            ret = importer->ops->import(importer, &glr->texture, &glr->width,
                                        &glr->height, r->p_picture, &pixels_offset);
        }
    }
                                ...
                                const video_format_t *source)
{
     ...
     || source->i_visible_height != vgl->last_source.i_visible_height)
    {
        ...
        for (unsigned j = 0; j < tc->importer.tex_count; j++)
        {
            float scale_w = (float)tc->importer.cfg[j].w.num / tc->importer.cfg[j].w.den
                          / vgl->tex_width[j];
            float scale_h = (float)tc->importer.cfg[j].h.num / tc->importer.cfg[j].h.den
                          / vgl->tex_height[j];
