...
CreateFilters(vlc_gl_t *gl, const struct vlc_gl_api *api,
              struct vlc_gl_interop *interop, const char *glfilters_config,
              struct vlc_gl_renderer **out_renderer)
{
    ...
    int ret = LoadFilters(gl, filters, glfilters_config, out_renderer);
    if (ret != VLC_SUCCESS)
    {
                                               ...
                                               const char *glfilters_config)
{
    ...
    vgl->filters = CreateFilters(gl, api, vgl->interop, glfilters_config,
                                 &vgl->renderer);
    if (!vgl->filters)
        goto delete_interop;
                                     ...
                                     const char *glfilters_config)
{
    ...
    struct vlc_gl_filters *filters =
        CreateFilters(gl, api, interop, glfilters_config, &renderer);
    if (!filters)
    {
