...
static void
InitStereoMatrix(GLfloat *matrix_out, video_multiview_mode_t multiview_mode)
{
    /*
...
#undef GET_ALOC
    ...
    int ret = vlc_gl_sampler_Fetch(sampler, program_id);
    assert(ret == VLC_SUCCESS);
    if (ret != VLC_SUCCESS)
    {
        msg_Err(renderer->gl, "Unable to get locations from sampler");
        goto error;
    }

    ret = sampler->pf_fetch_locations(sampler, program_id);
    assert(ret == VLC_SUCCESS);
    if (ret != VLC_SUCCESS)
    {
                    ...
                    const video_format_t *fmt, bool b_dump_shaders)
{
    ...
    struct vlc_gl_sampler *sampler = vlc_gl_sampler_New(interop, fmt);
    if (!sampler)
        return NULL;
