...
InitProgramYadif(struct vlc_gl_filter *filter)
{
    ...
    static const char *const FRAGMENT_SHADER =
        ...
        "  float d = (prev_pix + next_pix) / 2.0;\n"
        "  float e = pix(cur, x, y-1);\n"
        "  float temporal_diff0 = abs(prev_pix - cur_pix);\n"
        "  float temporal_diff1 = abs(pix(prev, x, y+1) - c)\n"
        "                       + abs(pix(prev, x, y-1) - e);\n"
        "  float temporal_diff2 = abs(pix(next, x, y+1) - c)\n"
        "                       + abs(pix(prev, x, y-1) - e);\n"
        "  float diff = max(temporal_diff0 / 2.0,\n"
        ...
        "    spatial_score = score;\n"
        "    spatial_pred = pix(cur, x-1, y+1) + pix(cur, x+1, y-1);\n"
        "    score = compute_score(x, y, -2);\n"
        "    if (score < spatial_score) {\n"
        "       spatial_score = score;\n"
        "       spatial_pred = pix(cur, x-2, y+1) + pix(cur, x+2, y-1);\n"
        "    }\n"
        "  }\n"
        "  score = compute_score(x, y, 1);\n"
        "  if (score < spatial_score) {\n"
        ...
        "  float result;\n"
        "  if (mod(y, 2.0) == 0.0) {\n"
        "    result = texture2D(cur, tex_coords).x;\n"
        "  } else {\n"
        "    result = filter(x, y);\n"
        "  }\n"
