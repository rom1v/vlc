...
static picture_t *Swap(vlc_gl_t *gl)
{
    ...
    struct pbo_picture_context *context = &sys->picture_contexts[sys->current_flip];
    ...
    //msg_Info(gl, "Current flip is %d", sys->current_flip);
...
{
    struct vlc_gl_pbuffer *sys = malloc(sizeof *sys);
    if (sys == NULL)
        return VLC_ENOMEM;
