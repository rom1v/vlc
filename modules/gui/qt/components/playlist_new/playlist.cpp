...
void
Playlist::requestAppend(QVector<Media> media)
{
    PlaylistLocker locker(this);

    auto rawMedia = toRawMedia(media);
    int ret = vlc_playlist_AppendAll(playlist,
...
void
Playlist::requestInsert(size_t index, QVector<Media> media)
{
    PlaylistLocker locker(this);
    ...
    auto rawMedia = toRawMedia(media);
    int ret = vlc_playlist_InsertAll(playlist, index,
                                     rawMedia.constData(), rawMedia.size());
    if (ret != VLC_SUCCESS)
        throw std::bad_alloc();
}
