...
PlaylistModel::onPlaylistItemsMoved(size_t index, size_t count, size_t target)
{
    ...
    /* QVector does not expose a method to move a slice of items, so
     * remove then reinsert */
    QVector<PlaylistItem> tmp;
    tmp.resize(count);
    std::move(items.cbegin() + index, items.cbegin() + index + count,
              tmp.begin());
    items.remove(index, count);

    items.insert(target, count, nullptr);
    std::move(tmp.cbegin(), tmp.cend(), items.begin() + target);
