               ...
               : QWidget( _par ), p_intf ( _p_i )
{
    ...
    media_source_provider_t *mediaSourceProvider = media_source_provider_Get( p_intf->obj.libvlc );
    media_source_t *source = media_source_provider_GetMediaSource( mediaSourceProvider, "video_dir" );
    QObject::connect( view, &QObject::destroyed, [source] {
        media_source_Release( source );
    });
    MediaTreeModel *mspModel = new MediaTreeModel( p_intf, source->p_tree, view );
    view->setModel( mspModel );
