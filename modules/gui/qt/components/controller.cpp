                   ...
                   : QFrame( _parent )
{
    ...
    connect( THEMIM, &InputManager::playingStateChanged, this, &AbstractController::setStatus);

    setSizePolicy( QSizePolicy::MinimumExpanding, QSizePolicy::Fixed );
}

/* Reemit some signals on status Change to activate some buttons */
void AbstractController::setStatus( InputManager::PlayingState status )
{
    bool b_hasInput = THEMIM->hasInput();
    /* Activate the interface buttons according to the presence of the input */
    emit inputExists( b_hasInput );

    emit inputPlaying( status == InputManager::PLAYING_STATE_PLAYING );
...
#define ENABLE_ON_VIDEO( a ) \
    connect( THEMIM, &InputManager::hasVideoOutputChanged, a, &QToolButton::setEnabled ); \
...
QWidget *AbstractController::createWidget( buttonType_e button, int options )
{
    ...
    case PLAY_BUTTON: {
        ...
        playButton->updateButtonIcons( THEMIM->getPlayingState() == InputManager::PLAYING_STATE_PLAYING );
        widget = playButton;
        }
        break;
    case STOP_BUTTON:{
    ...
    case NEXT_BUTTON: {
        ...
        break;
    case SLOWER_BUTTON:{
        NORMAL_BUTTON( SLOWER, THEMIM, &InputManager::slower );
        ENABLE_ON_INPUT( SLOWERButton );
        }
        break;
    case FASTER_BUTTON:{
        NORMAL_BUTTON( FASTER, THEMIM, &InputManager::faster );
    ...
    case PREV_SLOW_BUTTON:{
        ...
        connect( but, &QToolButtonExt::longClicked, THEMIM, &InputManager::jumpBwd );
        widget = but;
        }
        break;
    case NEXT_FAST_BUTTON:{
        ...
        connect( but, &QToolButtonExt::longClicked, THEMIM, &InputManager::jumpFwd );
        widget = but;
        }
        break;
    case FRAME_BUTTON: {
        NORMAL_BUTTON( FRAME, THEMIM, &InputManager::frameNext );
    ...
    case DEFULLSCREEN_BUTTON:
        {
        NORMAL_BUTTON( FULLSCREEN, THEMIM, &InputManager::toggleFullscreen );
    ...
    case SNAPSHOT_BUTTON:{
        NORMAL_BUTTON( SNAPSHOT, THEMIM, &InputManager::snapshot );
    ...
    case RECORD_BUTTON:{
        ...
        connect( recordButton, &QToolButton::toggled, THEMIM, &InputManager::setRecording );
        connect( THEMIM, &InputManager::recordingChanged, recordButton, &QToolButton::setChecked );
    ...
    case ATOB_BUTTON: {
        ...
        connect( ABButton, &AtoB_Button::clicked, THEMIM, &InputManager::toggleABloopState );
        connect( THEMIM, &InputManager::ABLoopStateChanged, ABButton, &AtoB_Button::updateButtonIcons );
        widget = ABButton;
        }
        break;
    case INPUT_SLIDER: {
    ...
    case REVERSE_BUTTON:{
        ...
        connect( reverseButton, &QToolButton::clicked, THEMIM, &InputManager::reverse);
    ...
    case SKIP_BACK_BUTTON: {
        NORMAL_BUTTON( SKIP_BACK, THEMIM, &InputManager::jumpBwd );
        ENABLE_ON_INPUT( SKIP_BACKButton );
        }
        break;
    case SKIP_FW_BUTTON: {
        NORMAL_BUTTON( SKIP_FW, THEMIM, &InputManager::jumpFwd  );
...
QFrame *AbstractController::discFrame()
{
    ...
    /* Change the navigation button display when the IM
       navigation changes */
    connect( THEMIM, &InputManager::hasTitlesChanged, chapFrame, &QFrame::setVisible );
    connect( THEMIM, &InputManager::hasMenuChanged, menuFrame, &QFrame::setVisible );

    /* Changes the IM navigation when triggered on the nav buttons */
    connect( prevSectionButton, &QToolButton::clicked, THEMIM, &InputManager::sectionPrev );
    connect( nextSectionButton, &QToolButton::clicked, THEMIM, &InputManager::sectionNext );
    connect( menuButton, &QToolButton::clicked, THEMIM, &InputManager::sectionMenu );
...
QFrame *AbstractController::telexFrame()
{
    ...
    connect( THEMIM,  &InputManager::teletextAvailableChanged, telexFrame, &QFrame::setVisible );
    ...
    connect( telexOn, &QToolButton::clicked, THEMIM, &InputManager::enableTeletext );
    connect( THEMIM, &InputManager::teletextEnabledChanged , telexOn, &QToolButton::setChecked );
    ...
    connect( telexTransparent, &QToolButton::clicked, THEMIM, &InputManager::setTeletextTransparency );
    connect( THEMIM, &InputManager::teletextTransparencyChanged, telexTransparent, &QToolButton::setChecked );
...
#define CREATE_CONTEXT_BUTTON(color, key) \
    ...
    connect( contextButtonMapper, QOverload<int>::of(&QSignalMapper::mapped), THEMIM, &InputManager::setTeletextPage );\
    connect( THEMIM, &InputManager::teletextEnabledChanged, contextButton, &QToolButton::setEnabled );\
...
#undef CREATE_CONTEXT_BUTTON
    ...
    /* Page change and set */
    connect( telexPage, QOverload<int>::of(&QSpinBox::valueChanged), THEMIM, &InputManager::setTeletextPage );
    connect( THEMIM, &InputManager::teletextPageChanged, telexPage, &QSpinBox::setValue);

    connect( THEMIM, &InputManager::teletextEnabledChanged, telexPage, &QSpinBox::setEnabled);
    connect( THEMIM, &InputManager::teletextEnabledChanged, telexTransparent, &QSpinBox::setEnabled );
    return telexFrame;
}
                           ...
                           : AbstractController( _p_i, _parent )
{
    ...
    connect( THEMIM, &InputManager::voutListChanged,
