...
void ExtVideo::cropChange()
{
    ...
    InputManager::VoutPtrList p_vouts = THEMIM->getVouts();
...
{
    InputManager::AoutPtr p_aout = THEMIM->getAout();
    ...
    , m_SubsDelayCfgFactor(VLC_OBJECT(p_intf), SUBSDELAY_CFG_FACTOR)
{
    ...
    connect( THEMIM, &InputManager::subtitleDelayChanged, [this](vlc_tick_t value) {
        b_userAction = false;
        subsSpin->setValue(secf_from_vlc_tick(value));
        b_userAction = true;
    });
    connect( THEMIM, &InputManager::audioDelayChanged, [this](vlc_tick_t value) {
        b_userAction = false;
        AVSpin->setValue(secf_from_vlc_tick(value));
        b_userAction = true;
    });
    connect( THEMIM, &InputManager::subtitleFPSChanged, subSpeedSpin, &QDoubleSpinBox::setValue );
