        ...
        , p_edp( NULL )
{
    ...
    connect( THEMIM, &InputManager::playingStateChanged, this, &ExtensionsManager::playingChanged );
...
void ExtensionsManager::playingChanged( InputManager::PlayingState state )
{
    //This is unlikely, but can happen if no extension modules can be loaded.
    if ( p_extensions_manager == NULL )
        return ;
