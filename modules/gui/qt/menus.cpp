...
static inline void VolumeEntries( intf_thread_t *p_intf, QMenu *current )
{
    ...
    current->addAction( QIcon( ":/toolbar/volume-high.svg" ), qtr( "&Increase Volume" ), THEMIM, &InputManager::setVolumeUp );
    current->addAction( QIcon( ":/toolbar/volume-low.svg" ), qtr( "D&ecrease Volume" ), THEMIM, &InputManager::setVolumeDown );
    current->addAction( QIcon( ":/toolbar/volume-muted.svg" ), qtr( "&Mute" ), THEMIM, &InputManager::toggleMuted );
}
...
QMenu *VLCMenuBar::VideoMenu( intf_thread_t *p_intf, QMenu *current )
{
    if( current->isEmpty() )
    {
        ...
        connect(THEMIM, &InputManager::hasVideoOutputChanged, action, &QAction::setEnabled);
        ...
        connect(snapshotAction, &QAction::triggered, THEMIM, &InputManager::snapshot);
...
#define ADD_ACTION( title, slot, visibleSignal, visible ) \
    ...
    ADD_ACTION( qtr("Previous Title"), &InputManager::titlePrev, &InputManager::hasTitlesChanged, THEMIM->hasTitles() );
    ADD_ACTION( qtr("Next Title"), &InputManager::titleNext, &InputManager::hasTitlesChanged, THEMIM->hasTitles() );
    ADD_ACTION( qtr("Previous Chapter"), &InputManager::chapterPrev, &InputManager::hasChaptersChanged, THEMIM->hasChapters() );
    ADD_ACTION( qtr("Next Chapter"), &InputManager::chapterNext, &InputManager::hasChaptersChanged, THEMIM->hasChapters() );
...
void VLCMenuBar::PopupMenuPlaylistEntries( QMenu *menu, intf_thread_t *p_intf )
{
    ...
    if( !hasInput || THEMIM->getPlayingState() != InputManager::PLAYING_STATE_PLAYING )
    {
    ...
    action = menu->addAction( qtr( "Record" ), THEMIM, &InputManager::toggleRecord );
...
QMenu* VLCMenuBar::PopupMenu( intf_thread_t *p_intf, bool show )
{
    ...
    {
        QMenu *submenu;
        InputManager::VoutPtr p_vout = THEMIM->getVout();
...
void VLCMenuBar::updateAudioDevice( intf_thread_t * p_intf, QMenu *current )
{
    ...
    current->clear();
    InputManager::AoutPtr aout = THEMIM->getAout();
    if (!aout)
        return;
