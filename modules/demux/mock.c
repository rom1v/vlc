...
CreateTitle(demux_t *demux, size_t idx)
{
    ...
    if (asprintf(&t->psz_name, "Title %zu", idx) == -1)
    {
        t->psz_name = NULL;
    ...
    for (ssize_t i = 0; i < sys->chapter_count; ++i)
    {
        ...
        t->i_seekpoint++;
        if (asprintf(&t->seekpoint[i]->psz_name, "Chapter %zu-%zu", idx, i)
            == -1)
        {
            vlc_input_title_Delete(t);
            return NULL;
...
static int
InitVideoTracks(demux_t *demux, int group)
{
    struct demux_sys *sys = demux->p_sys;
...
static int
InitAudioTracks(demux_t *demux, int group)
{
    struct demux_sys *sys = demux->p_sys;
...
static int
InitSubTracks(demux_t *demux, int group)
{
    struct demux_sys *sys = demux->p_sys;

    if (sys->sub_track_count == 0)
        return VLC_SUCCESS;

    for (ssize_t i = 0; i < sys->sub_track_count; ++i)
    {
        es_format_t fmt;
...
#undef X
    ...
    {
        ret = InitVideoTracks(demux, i);
        if (ret != VLC_SUCCESS)
            goto error;

        ret = InitAudioTracks(demux, i);
        if (ret != VLC_SUCCESS)
            goto error;

        ret = InitSubTracks(demux, i);
        if (ret != VLC_SUCCESS)
            goto error;
    }
