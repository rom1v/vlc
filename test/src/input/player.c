...
{
    struct vlc_player_track *unselected;
    struct vlc_player_track *selected;
};
...
{
    struct vlc_player_program *unselected;
    struct vlc_player_program *selected;
};
...
player_on_track_selection_changed(vlc_player_t *player,
                                  const struct vlc_player_track *unselected,
                                  const struct vlc_player_track *selected,
                                  void *data)
{
    struct ctx *ctx = get_ctx(player, data);
    struct report_track_selection report = {
        .unselected = unselected ? vlc_player_track_Dup(unselected) : NULL,
        .selected = selected ? vlc_player_track_Dup(selected) : NULL,
    };
    assert(!!unselected == !!report.unselected);
...
player_on_program_selection_changed(vlc_player_t *player,
                                    const struct vlc_player_program *unselected,
                                    const struct vlc_player_program *selected,
                                    void *data)
{
    struct ctx *ctx = get_ctx(player, data);
    struct report_program_selection report = {
        .unselected = unselected ? vlc_player_program_Dup(unselected) : NULL,
        .selected = selected ? vlc_player_program_Dup(selected) : NULL,
    };
    assert(!!unselected == !!report.unselected);
    assert(!!selected == !!report.selected);
...
#define CLEAN_MEDIA_VEC(vec) do { \
...

    {
        ...
        FOREACH_VEC(report, on_track_selection_changed)
        {
            if (report.unselected)
                vlc_player_track_Delete(report.unselected);
            if (report.selected)
                vlc_player_track_Delete(report.selected);
        }
    }
    ...
    {
        struct report_program_selection report;
        FOREACH_VEC(report, on_program_selection_changed)
        {
            if (report.unselected)
                vlc_player_program_Delete(report.unselected);
            if (report.selected)
                vlc_player_program_Delete(report.selected);
        }
    }
