...
struct libvlc_playlist
{
    ...
    libvlc_media_player_t *libvlc_player;
    bool owned;
...
libvlc_playlist_item_Release(libvlc_playlist_item_t *libvlc_item)
{
    if (vlc_atomic_rc_dec(&libvlc_item->rc))
        libvlc_playlist_item_Delete(libvlc_item);
}
...
libvlc_playlist_ClearAll(libvlc_playlist_t *playlist)
{
    for (size_t i = 0; i < playlist->items.size; ++i)
        libvlc_playlist_item_Release(playlist->items.data[i]);
                              ...
                              input_item_t *input_item, ssize_t index_hint)
{
    ...
    {
        libvlc_media_t *media = libvlc_playlist->user_media[i];
        if (media->p_input_item == input_item)
            return media;
    }
...
static libvlc_media_t **
LibvlcItemsToMedias(libvlc_playlist_item_t *const libvlc_items[], size_t count)
{
    libvlc_media_t **medias = vlc_alloc(count, sizeof(*medias));
    if (!medias)
        return NULL;

    for (size_t i = 0; i < count; ++i)
    {
        medias[i] = libvlc_items[i]->media;
        libvlc_media_retain(medias[i]);
    }
    return medias;
}

static void
DeleteMedias(libvlc_media_t *libvlc_media[], size_t count)
{
    for (size_t i = 0; i < count; ++i)
        libvlc_media_release(libvlc_media[i]);
    free(libvlc_media);
}
                             ...
                             libvlc_playlist_item_t *dest[])
{
    for (size_t i = 0; i < count; ++i)
    {
        ...
        if (!libvlc_item)
        {
             ...
             * the loop */
            while (i--)
                libvlc_playlist_item_Release(dest[i]);
            return false;
        }
                 ...
                 void *userdata)
{
    ...
    for (size_t i = 0; i < count; ++i)
        libvlc_playlist_item_Release(libvlc_playlist->items.data[index + i]);
    vlc_vector_remove_slice(&libvlc_playlist->items, index, count);
                 ...
                 void *userdata)
{
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        libvlc_playlist_item_Release(libvlc_playlist->items.data[i]);
                     ...
                     bool owned)
{
    ...
    vlc_list_init(&libvlc_playlist->listeners);

    vlc_player_t *player = vlc_playlist_GetPlayer(playlist);
    libvlc_playlist->libvlc_player =
        libvlc_media_player_NewWithPlayer(libvlc, player, false);
...
void
libvlc_playlist_Delete(libvlc_playlist_t *libvlc_playlist)
{
    vlc_playlist_t *playlist = libvlc_playlist->playlist;

    vlc_playlist_Lock(playlist);
    vlc_playlist_RemoveListener(playlist, libvlc_playlist->listener);
    vlc_playlist_Unlock(playlist);

    libvlc_media_player_release(libvlc_playlist->libvlc_player);

    libvlc_playlist_ClearAll(libvlc_playlist);
    vlc_vector_destroy(&libvlc_playlist->items);

    if (libvlc_playlist->owned)
        vlc_playlist_Delete(playlist);
    libvlc_release(libvlc_playlist->libvlc);
    free(libvlc_playlist);
}
...
libvlc_playlist_New(libvlc_instance_t *libvlc)
{
    ...
    vlc_playlist_t *playlist = vlc_playlist_New(obj, VLC_PLAYER_LOCK_REENTRANT);
...
{
    size_t size = libvlc_playlist->items.size;
    /* the original items will be released during on_items_reset */
    libvlc_media_t **medias =
        LibvlcItemsToMedias(libvlc_playlist->items.data, size);
    if (!medias)
        return -1;

    libvlc_playlist_SaveUserMedia(libvlc_playlist, medias, size);
    vlc_playlist_Shuffle(libvlc_playlist->playlist);
    libvlc_playlist_ResetUserMedia(libvlc_playlist);

    DeleteMedias(medias, size);
    return 0;
}
...
libvlc_playlist_GetPlaybackRepeat(libvlc_playlist_t *libvlc_playlist)
{
    vlc_playlist_t *playlist = libvlc_playlist->playlist;
    enum vlc_playlist_playback_repeat repeat =
        vlc_playlist_GetPlaybackRepeat(playlist);
    return CoreToLibvlcRepeat(repeat);
}

enum libvlc_playlist_playback_order
libvlc_playlist_GetPlaybackOrder(libvlc_playlist_t *libvlc_playlist)
{
    vlc_playlist_t *playlist = libvlc_playlist->playlist;
    enum vlc_playlist_playback_order order =
        vlc_playlist_GetPlaybackOrder(playlist);
    return CoreToLibvlcOrder(order);
}
...
libvlc_playlist_GetPlayer(libvlc_playlist_t *libvlc_playlist)
{
    return libvlc_playlist->libvlc_player;
}
